      ...
      if (/\.zip\//.test(binPath)) {
        ...
        if (pnpapi) {
          ...
          const binTargetPath = path2.join(
            ...
            ".cache",
            "esbuild",
            `pnpapi-${pkg.replace("/", "-")}-${"0.25.12"}-${path2.basename(subpath)}`
          );
...
var esbuild = __toESM(require_main2(), 1);
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
var __filename2 = fileURLToPath(import.meta.url);
var __dirname2 = path.dirname(__filename2);
var rootDir = path.resolve(__dirname2, "..");
import_dotenv.default.config({ path: path.join(rootDir, ".env") });
var production = process.argv.includes("--production");
var watch = process.argv.includes("--watch");
var aliasResolverPlugin = {
  name: "alias-resolver",
  setup(build) {
    const aliases = {
      ...
      build.onResolve({ filter: aliasRegex }, (args) => {
        ...
        if (fs.existsSync(importPath)) {
          ...
          if (stats.isDirectory()) {
            const extensions2 = [".ts", ".tsx", ".js", ".jsx"];
...
var vscodeStubPlugin = {
  name: "vscode-stub",
  setup(build) {
    build.onResolve({ filter: /^vscode$/ }, () => {
      return { path: path.join(__dirname2, "src", "vscode-shim.ts") };
    });
  }
};
var esbuildProblemMatcherPlugin = {
...
var stubOptionalModulesPlugin = {
  name: "stub-optional-modules",
  setup(build) {
    build.onResolve({ filter: /^react-devtools-core$/ }, () => {
      return { path: path.join(__dirname2, "src", "stub-devtools.js"), external: false };
    });
  }
};
var copyWasmFiles = {
  ...
  setup(build) {
    build.onEnd(() => {
      const destDir = path.join(__dirname2, "dist");
...
var sharedOptions = {
  ...
  logLevel: "silent",
  define: buildEnvVars,
  tsconfig: path.join(__dirname2, "tsconfig.json"),
  ...
  external: [
    ...
    "pino-roll",
    "@vscode/ripgrep"
    // Uses __dirname to locate the binary
  ],
  supported: { "top-level-await": true }
};
var cliConfig = {
  ...sharedOptions,
  entryPoints: [path.join(__dirname2, "src", "index.ts")],
  outfile: path.join(__dirname2, "dist", "cli.mjs"),
  banner: {
    js: `#!/usr/bin/env node
...
var libConfig = {
  ...sharedOptions,
  entryPoints: [path.join(__dirname2, "src", "exports.ts")],
  outfile: path.join(__dirname2, "dist", "lib.mjs"),
  banner: {
...
async function main() {
  if (watch) {
    ...
    const cliOutfile = path.join(__dirname2, "dist", "cli.mjs");
